...
let overlay = document.getElementById("videoPlayOverlay");
...
// when I click on the overlay, the video doesn't play or pause, so I added an event listener on the overlay so that when I click on it, it has the same effect as the play button.
...
function setTime() {
  let minute = Math.floor (videoElement.currentTime / 60);
  let second = Math.floor (videoElement.currentTime - minute*60);

  minute = minute.toString().padStart(2,"0");
  second = second.toString().padStart(2,"0");

  let mediaTime = `${minute}:${second}`;
  timerSpan.textContent = mediaTime;
}


// Volume Slider
volSlider.addEventListener("input",(e) => {newVolume(e.target.value)});

function newVolume(newVolume) {
  videoElement.volume=newVolume;
  if (videoElement.volume == 0){
    volumeButton.style.backgroundImage = "url('./icons/unmute.svg')";
  }
  else {
    volumeButton.style.backgroundImage = "url('./icons/mute.svg')";
  }
}

//Volume Button
volumeButton.addEventListener("click",(e) => {muteUnmute()});

function muteUnmute(){
  if (mute == false){
    var returnVolume = videoElement.volume;
    console.log(returnVolume)
